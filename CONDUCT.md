...
>
> **Progress:** Investigated `SetHelpCommand` — you can replace the default `help` subcommand by calling `cmd.SetHelpCommand(customCmd)` on the root command before `Execute()`. The custom command must handle the case where a subcommand name is passed as an argument (i.e. `myapp help subcommand`). If you just want to hide the help command without replacing it, set a command with `Hidden: true` instead of removing it entirely.
>
> **Progress:** Investigated `Args` validators — built-ins include `NoArgs`, `ArbitraryArgs`, `MinimumNArgs(n)`, `MaximumNArgs(n)`, `ExactArgs(n)`, and `RangeArgs(min, max)`. You can also pass a custom `func(cmd *cobra.Command, args []string) error` directly to the `Args` field. Note: if `Args` is nil, Cobra defaults to `ArbitraryArgs` (no validation), so it's good practice to always set an explicit validator.
